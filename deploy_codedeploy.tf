resource "local_file" "appspec" {
  ...
  filename = "./${aws_codedeploy_app.main[count.index].name}.yaml"
...
resource "null_resource" "deploy_codedeploy" {
  ...
  provisioner "local-exec" {
    command = "aws deploy create-deployment --cli-input-yaml file://./${aws_codedeploy_app.main[count.index].name}.yaml"
    environment = {
